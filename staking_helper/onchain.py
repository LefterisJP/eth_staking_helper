...
def stakefish_deposit(
    ...
    if batch_deposit.functions.fee().call() != 0:
        error(
            "Stakefish has activated their fee. You can find better batching solutions"
        )
        return
    ...
    arguments = {
        ...
        "withdrawal_credentials": bytes.fromhex(
            withdrawal_credentials.removeprefix("0x")
        ),
        "signatures": bytes.fromhex(signatures.removeprefix("0x")),
        "deposit_data_roots": [
            bytes.fromhex(x.removeprefix("0x")) for x in deposit_data_roots
        ],
    }
